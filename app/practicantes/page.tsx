...
export default function FormAddPractica() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const id = await addDocument("practicas", data); // Asegúrate de que "practicas" sea la colección correcta
      alert("Práctica agregada con éxito" + (id ? ` con ID: ${id}` : ""));

      // Agregar también a Supabase
      const { data: supabaseData, error: supabaseError } = await supabase
        .from("practicas") // Asume que tienes una tabla llamada 'practicas' en Supabase
        .insert([data]);
  ...
  return (
    <main className="max-w-lg mx-auto p-6 bg-white rounded-lg shadow-md mt-6">
      <h1 className="text-2xl font-bold mb-6 text-center">Agregar Práctica Para Practicantes</h1>
